      ...
      return (
        <Link
          ...
          className={cn(
            "group relative flex items-center gap-3 overflow-hidden rounded-md transition-colors",
              ...
              isActive && (isCollapsed ? "bg-transparent text-foreground" : "bg-accent text-accent-foreground"),
        ...
        >
          ...
          {item.badge && !isCollapsed ? (
            <span className="ml-auto rounded-md border border-primary/20 bg-primary/10 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.12em] text-primary">
              {item.badge}
            </span>
          ) : null}
        </Link>
  ...
  return (
    <div className="flex h-full flex-col">
      <div
        className={cn(
          "px-4 pt-5",
          isCollapsed ? "pb-2" : "border-b border-border pb-4",
        )}
      >
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 overflow-hidden">
            <div
              className={cn(
                "grid h-9 w-9 place-items-center rounded-md text-primary-foreground",
                isCollapsed ? "bg-transparent" : "bg-primary",
              )}
            >
            ...
            <div className={cn("leading-tight", isCollapsed && "hidden")}>
              <p className="text-lg font-semibold tracking-[0.12em] text-foreground">EXPLORER</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-border px-3 py-4">
        <div className={cn("space-y-2", isCollapsed && "space-y-3") }>
          <Link
            ...
            rel="noreferrer"
            className={cn(
              "inline-flex items-center gap-2 rounded-md text-muted-foreground transition-colors hover:text-foreground",
              isCollapsed ? "h-9 w-full justify-center hover:bg-accent" : "h-9 px-2",
            )}
          >
...
export function Sidebar() {
  ...
  return (
    <>
      <div
        className={cn(
          "fixed inset-0 z-40 bg-primary/20 backdrop-blur-sm transition-opacity lg:hidden",
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 hidden border-r border-border bg-background/90 backdrop-blur-xl lg:flex",
      ...
      >
        ...
        <button
          ...
          className={cn(
            "absolute top-6 z-10 inline-flex h-6 w-6 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition-colors hover:text-foreground",
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-64 border-r border-border bg-background/90 backdrop-blur-xl transition-transform duration-300 lg:hidden",
      ...
      >
        ...
        <button
          ...
          className="absolute right-3 top-6 inline-flex h-7 w-7 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition-colors hover:text-foreground"
        >
